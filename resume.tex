...
\begin{center}
  ...
  \par
  \vspace{1mm}
  \large{chislett.ben@gmail.com} $|$ \large{bchislett@nvidia.com}
  \par
...
\textit{NVIDIA, Toronto, ON}
\begin{itemize}
  \item Actively maintaining and developing vLLM as a core committer responsible for speculative decoding, structured outputs, and asynchronous scheduling features
  \item Delivered key performance optimizations for vLLM, such as overlapped execution for asynchronous scheduling and both kernel-level and algorithmic optimizations for speculative decoding
  \item Performing original research for novel speculative decoding techniques, and implementing new techniques from latest literature into vLLM and training infrastructure \\
\end{itemize}

\textbf{Software Engineer}
\hfill
\textit{Sep 2024 - Jun 2025}\\*
\textit{CentML, Toronto, ON}
\begin{itemize}
  \item Developed targeted optimizations on top of vLLM for LLM inference workloads
  \item Contributed major features to vLLM, with a focus on speculative decoding and vision-language model support
  \item Acquired by NVIDIA in June 2025 \\
\end{itemize}
...
\begin{itemize}
  ...
  \item Developed custom high utilization hardware-accelerated machine learning primitives for differentiable rendering in PTX/CUDA
  \item Left the program after first year to pursue applied opportunities in industry \\
\end{itemize}
